...
function ResourceGanttRow({ resourceName, items, timeline }) {
  return <div className="grid grid-cols-[240px_1fr] items-center gap-5"><div className="text-left"><p className="font-semibold text-slate-900">{resourceName}</p><p className="mt-1 text-xs text-slate-500">{items.length} assignment{items.length === 1 ? "" : "s"}</p></div><div className="relative h-11 rounded-xl bg-slate-100">{items.map((item) => <GanttSegmentBar key={`${resourceName}-${item.id}`} item={item} timeline={timeline} label={item.project.name} />)}</div></div>;
}
