...
const ASSIGNMENTS_KEY = "ggc_resource_planning_assignments_v1";
...
function getProjectCrews(assignment) { return [assignment.crew1, assignment.crew2, assignment.crew3, assignment.crew4].filter(Boolean); }
function getAssignmentPeopleLabel(assignment) { return [assignment.superintendent, assignment.crew1, assignment.crew2, assignment.crew3, assignment.crew4].filter(Boolean).join(" • "); }
...
function AssignmentForm({ form, setForm, onSave, onCancel, editing, resources, projects }) {
  function updateField(field, value) { setForm((current) => ({ ...current, [field]: value })); }
...
function ProjectGanttRow({ assignment, project, items, timeline }) {
  const label = getAssignmentPeopleLabel(assignment);
...
function countRequiredResources(assignment) {
  return [assignment.projectManager, assignment.superintendent, assignment.fieldCoordinator, assignment.fieldEngineer, assignment.safety, assignment.crew1, assignment.crew2, assignment.crew3, assignment.crew4].filter(Boolean).length;
}
...
export default function App() {
  ...
  const crews = [...new Set(assignments.flatMap((assignment) => getProjectCrews(assignment)).filter(Boolean))];
  ...
  return <main className="min-h-screen bg-slate-50 text-slate-900"><header className="border-b border-slate-200 bg-white"><div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-5"><div><div className="h-1 w-24 rounded-full bg-emerald-700" /><h1 className="mt-3 text-3xl font-bold tracking-tight">GGC Resource Planning</h1><p className="mt-1 text-slate-500">Project master list, resource assignments, and mobilization scheduling.</p></div><div className="flex flex-wrap justify-end gap-3"><button onClick={() => setPage("dashboard")} className={`rounded-xl px-4 py-3 font-semibold shadow-sm ${page === "dashboard" ? "bg-slate-900 text-white" : "border border-slate-300 bg-white text-slate-700 hover:bg-slate-50"}`}>Dashboard</button><button onClick={() => setPage("projects")} className={`rounded-xl px-4 py-3 font-semibold shadow-sm ${page === "projects" ? "bg-slate-900 text-white" : "border border-slate-300 bg-white text-slate-700 hover:bg-slate-50"}`}>Projects</button><button onClick={() => setPage("resources")} className={`rounded-xl px-4 py-3 font-semibold shadow-sm ${page === "resources" ? "bg-slate-900 text-white" : "border border-slate-300 bg-white text-slate-700 hover:bg-slate-50"}`}>Resources</button>{page === "dashboard" ? <button onClick={openAddAssignmentForm} className="flex items-center gap-2 rounded-xl bg-emerald-700 px-4 py-3 font-semibold text-white shadow-sm hover:bg-emerald-800"><ClipboardCheck size={18} /> Assign</button> : page === "projects" ? <button onClick={openAddProjectForm} className="flex items-center gap-2 rounded-xl bg-emerald-700 px-4 py-3 font-semibold text-white shadow-sm hover:bg-emerald-800"><Plus size={18} /> Add Project</button> : <button onClick={openAddResourceForm} className="flex items-center gap-2 rounded-xl bg-emerald-700 px-4 py-3 font-semibold text-white shadow-sm hover:bg-emerald-800"><Plus size={18} /> Add Resource</button>}</div></div></header>
    ...
    {showAssignmentForm && <AssignmentForm form={assignmentForm} setForm={setAssignmentForm} onSave={saveAssignment} onCancel={() => setShowAssignmentForm(false)} editing={Boolean(editingAssignmentId)} resources={resources} projects={projects} />}
