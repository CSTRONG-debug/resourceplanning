// src/components/CmicApplyWipOverrides.jsx
//
// Renders the "Apply WIP Contract Overrides" button for the Forecast tab.
// Pulls the PREVIOUS calendar month's posted WIP, reads each job's contract
// override (falling back to the WIP contract amount), and diffs against the
// current forecast contract value. Current values are kept as the source of
// truth — only projects that have a posted WIP value and differ are surfaced
// and pre-checked. Apply writes new values to the `forecast` table (where
// contract values live in this app — NOT on the projects table).
//
// This is a deliberate near-clone of CmicRefreshContracts: same diff →
// checklist → apply-to-forecast flow, different source (posted WIP vs live
// job file) and a completion summary at the end.

import React, { useState } from "react";
import { createPortal } from "react-dom";
import { ClipboardCheck, X, Check, Loader2, AlertCircle } from "lucide-react";
import { supabase } from "../lib/supabase";
import { fetchWipContractOverrides } from "../lib/cmic";

export default function CmicApplyWipOverrides({ projects, forecastData, onApplied }) {
  ...
  async function handleClick() {
    ...
    setBusy(true);
    try {
      // Inject current contract value from the forecast map so the diff knows
      // what it's comparing against — same pattern as the contract refresh.
      ...
      const result = await fetchWipContractOverrides(candidates);
      setUpdates(result);
      setPeriodLabel(result[0]?.periodLabel || "");
      setInclude(
  ...
  async function handleApply() {
    ...
    try {
      ...
      setUpdates(null);
      onApplied?.({ updated: toApply.length, source: "wip-override", period: periodLabel });
      alert(
        `WIP contract overrides applied.\n\n` +
        `${toApply.length} project${toApply.length === 1 ? "" : "s"} updated from posted WIP (${periodLabel}).`
      );
    } catch (err) {
  ...
  return (
    <>
      <button
        ...
        title="Apply previous-period posted WIP contract overrides"
      >
        {busy ? <Loader2 size={18} className="animate-spin" /> : <ClipboardCheck size={18} />}
        {busy ? "Checking WIP…" : "Apply WIP Contract Overrides"}
      </button>
        ...
        >
          ...
          >
            <header className="flex shrink-0 items-center justify-between border-b border-slate-200 px-5 py-4">
              <div>
                <h2 className="text-lg font-bold text-slate-900">WIP Contract Overrides</h2>
                <p className="text-xs text-slate-500">
                  Posted WIP for {periodLabel} · {changedCount} project{changedCount === 1 ? "" : "s"} differ
                </p>
              </div>
              <button
                onClick={() => { setUpdates(null); setError(""); }}
            ...
            <footer className="flex shrink-0 items-center justify-between border-t border-slate-200 px-5 py-4">
              <div className="text-xs text-slate-500">
                Will update <strong>{includedCount}</strong> contract value{includedCount === 1 ? "" : "s"}
              </div>
